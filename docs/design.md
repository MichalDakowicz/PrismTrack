...
- All open PRs in linked repos surface in PrismTrack - PR card shows: title, author, target branch, CI status (pass/fail/pending), review status, merge status
...
_Spec version 1.1 — styling defined separately in `design.md`. Both files prepared for handoff to Google Stitch._
